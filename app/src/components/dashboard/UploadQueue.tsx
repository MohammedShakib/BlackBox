...
export function UploadQueue({ items, onClearFinished, onCancelAll, onCancelItem, onRetryItem }: UploadQueueProps) {
    ...
    return (
        <div className="fixed bottom-4 right-4 w-80 bg-blackbox-surface border border-blackbox-border rounded-xl shadow-2xl overflow-hidden z-[100]">
            <div className="p-3 border-b border-blackbox-border bg-blackbox-hover flex justify-between items-center">
                <h4 className="text-sm font-medium text-blackbox-text">Uploads</h4>
                <div className="flex gap-2">
                    {hasPendingOrActive && (
                        <button onClick={onCancelAll} className="text-xs text-red-400 hover:text-red-300 transition-colors">Cancel All</button>
                    )}
                    <button onClick={onClearFinished} className="text-xs text-blackbox-primary hover:text-blackbox-text transition-colors">Clear Finished</button>
                </div>
            </div>
                    ...
                    <div key={item.id} className="flex flex-col gap-1 p-2 bg-blackbox-hover rounded">
                        <div className="flex items-center gap-3 text-sm">
                            <div className={`w-2 h-2 rounded-full flex-shrink-0 ${item.status === 'pending' ? 'bg-yellow-500' :
                                item.status === 'uploading' ? 'bg-blackbox-ocean-green animate-pulse' :
                                    item.status === 'cancelled' ? 'bg-gray-500' :
                                        item.status === 'error' ? 'bg-red-500' : 'bg-green-500'
                                }`} />
                            <div className="flex-1 truncate text-blackbox-subtext" title={item.path}>
                                {item.path.split('/').pop()}
                            </div>
                            ...
                            {(item.status === 'error' || item.status === 'cancelled') && (
                                <button onClick={() => onRetryItem(item.id)} className="text-gray-400 hover:text-blackbox-ocean-green transition-colors flex-shrink-0" title="Retry">
                            ...
                            <>
                                <div className="w-full bg-blackbox-border h-1 mt-1 rounded-full overflow-hidden">
                                    {item.progress !== undefined ? (
                                        <div
                                            className="bg-blackbox-ocean-green h-full rounded-full transition-all duration-300"
                                            style={{ width: `${item.progress}%` }}
                                        />
                                    ) : (
                                        <div className="bg-blackbox-ocean-green h-full w-full animate-progress-indeterminate" />
                                    )}
                                </div>
