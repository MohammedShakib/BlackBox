...
export function DownloadQueue({ items, onClearFinished, onCancelAll, onCancelItem, onRetryItem }: DownloadQueueProps) {
    ...
    return (
        <div className="fixed bottom-4 right-2 sm:right-4 w-[calc(100vw-1rem)] sm:w-80 max-w-80 bg-blackbox-surface border border-blackbox-border rounded-xl shadow-2xl overflow-hidden z-[100]">
            <div className="p-3 border-b border-blackbox-border bg-blackbox-hover flex justify-between items-center">
                <div className="flex items-center gap-2">
                    <Download className="w-4 h-4 text-blackbox-secondary" />
                    <h4 className="text-sm font-medium text-blackbox-text">Downloads</h4>
                    {activeCount > 0 && (
                        <span className="text-xs px-1.5 py-0.5 bg-blackbox-secondary/20 text-blackbox-secondary rounded-full">
                            {activeCount} active
                        </span>
                    )}
                </div>
                <div className="flex gap-2">
                    ...
                    {completedCount > 0 && (
                        <button onClick={onClearFinished} className="text-xs text-blackbox-primary hover:text-blackbox-text transition-colors">
                            Clear Finished
                        </button>
                    )}
                </div>
            ...
            <div className="max-h-60 overflow-y-auto p-2 space-y-2">
                {items.map(item => (
                    <div key={item.id} className="flex flex-col gap-1 p-2 bg-blackbox-hover rounded">
                        <div className="flex items-center gap-3 text-sm">
                            <div className="flex-shrink-0">
                                ...
                                {item.status === 'downloading' && <div className="w-4 h-4 rounded-full border-2 border-blackbox-secondary border-t-transparent animate-spin" />}
                            ...
                            <div className="flex-1 truncate text-blackbox-subtext" title={item.filename}>
                                {item.filename}
                            </div>
                            {item.cacheInfo && (
                                <span className="text-xs text-blackbox-primary flex-shrink-0">{item.cacheInfo}</span>
                            )}
                            ...
                            {(item.status === 'error' || item.status === 'cancelled') && (
                                <button onClick={() => onRetryItem(item.id)} className="text-gray-400 hover:text-blackbox-primary transition-colors flex-shrink-0" title="Retry">
                            ...
                            <>
                                <div className="w-full bg-blackbox-border h-1 mt-1 rounded-full overflow-hidden">
                                    {item.progress !== undefined ? (
                                        <div
                                            className="bg-blackbox-secondary h-full rounded-full transition-all duration-300"
                                            style={{ width: `${item.progress}%` }}
                                        />
                                    ) : (
                                        <div className="bg-blackbox-secondary h-full w-full animate-progress-indeterminate" />
                                    )}
                                </div>
