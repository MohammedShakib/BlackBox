    ...
    return (
        ...
        >
            <motion.div
                ...
                whileHover={{ y: -4 }}
                className={`group cursor-pointer bg-blackbox-surface rounded-xl overflow-hidden border hover:shadow-[0_4px_20px_rgba(0,0,0,0.2)] transition-all relative
                ${isSelected ? 'border-blackbox-primary bg-blackbox-primary/5 ring-1 ring-blackbox-primary' : 'border-blackbox-border hover:border-blackbox-primary/50'}
                ${isDragOver ? 'ring-2 ring-blackbox-primary bg-blackbox-primary/20 scale-105' : ''}`}
            ...
            >
                ...
                {thumbnail ? (
                    ...
                    <div className="absolute inset-0 flex items-center justify-center p-4">
                        {isFolder ? (
                            <Folder className="w-12 h-12 text-blackbox-primary" />
                        ) : thumbnailLoading && isImageFile(file.name) ? (
                            <div className="w-8 h-8 border-2 border-blackbox-primary/30 border-t-blackbox-primary rounded-full animate-spin" />
                        ) : (
                ...
                <div
                    ...
                    className={`absolute top-2 left-2 w-5 h-5 rounded-full border flex items-center justify-center transition-all z-10 cursor-pointer ${isSelected ? 'bg-blackbox-primary border-blackbox-primary' : 'border-white/50 bg-black/30 opacity-0 group-hover:opacity-100'}`}
                >
                ...
                <div className={`absolute bottom-0 left-0 right-0 p-3 ${thumbnail ? 'text-white' : 'text-blackbox-text'}`}>
                    <h3 className="text-sm font-medium truncate w-full" title={file.name}>{file.name}</h3>
                    <p className={`text-xs mt-0.5 ${thumbnail ? 'text-white/70' : 'text-blackbox-subtext'}`}>{file.sizeStr}</p>
                </div>
